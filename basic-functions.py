...
print(close_prices.mean()) # prints mean stock price for each stock
print(close_prices.median()) # prints median stock price for each stock
print(close_prices.std()) # prints standard deviation of stock price for each stock
...
print(daily_return.mean()) # prints mean daily return for each stock
print(daily_return.std()) # prints standard deviation of daily returns for each stock

# Rolling mean and standard deviation
print(daily_return.rolling(window=20).mean()) # simple moving average
print(daily_return.rolling(window=20).std())

print(daily_return.ewm(span=20,min_periods=20).mean()) # exponential moving average
print(daily_return.ewm(span=20,min_periods=20).std())
